...
from hub_studio.reply import get_keyboard
...
@admin_router.message(StateFilter(None), F.text == "Добавить товар")  # Проверка на активное состояние StateFilter(None)
async def add_product(message: types.Message, state: FSMContext):
    await message.answer(
        "Введите название товара", reply_markup=types.ReplyKeyboardRemove()
    )
    await state.set_state(AddProduct.name)  # Становимся в состояние ожидания ввода AddProduct.name
